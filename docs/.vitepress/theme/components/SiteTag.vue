<template>
  <div class="e-site">
    <a class="e-site-link" :href="link" target="_blank" rel="noreferrer">
      <article class="box" :class="{'has-badge':formatBadge}">
        <div class="box-header">
          <template v-if="!noIcon">
            <span v-if="svg" class="icon" v-html="svg"></span>
            <span v-else-if="icon && typeof icon==='string'" class="icon">
              <img :src="withBase(icon)" :alt="title" onerror="this.parentElement.style.display='none'"/>
            </span>
          </template>
          <h4 v-if="title" :id="formatTitle" class="title" :class="{'no-icon':noIcon}">{{  title}}</h4>
        </div>
...
const props=defineProps<{
  noIcon?:boolean,
  icon?: SiteLinkTag['icon'],
  title?: SiteLinkTag['title'],
  desc?: SiteLinkTag['desc'],
  link: SiteLinkTag['link'],
  badge?: SiteLinkTag['badge'],
}>();
